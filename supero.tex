...
In order to simplify the example, we begin supercompilation from the expression |map (\b -> b + 1) as|, rather than |main|. Supercompilation proceeds by first applying |psi\!!(map (\b -> b + 1) as)|, to generate a fresh name, which we choose to be |h_0|. This new function |h_0| then has the free variables of the original expression as arguments, namely |as|. We then apply |drive| to the RHS, first inlining the |map| application, then applying the simplification rules:
...
generalise rho x = ELet2 bind x'
    where (x',bind,_) = msg x (head [y | y <<| x, y <- rho])
\end{code}
...
\begin{code}
generalise rho x = head [y | y <<| x, y <- rho] |><| x
\end{code}
