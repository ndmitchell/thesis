...
Having applied all the rules, we now classify what the parent expressions of a lambda may be. A lambda may not be the root of a function body, because |lam| cannot be generated as the root by |s|. If we focus only on those expressions which permit a lambda expression as a direct child, hiding the other expressions with |...|, we are left with the following upper bound for the set of residual expressions:
