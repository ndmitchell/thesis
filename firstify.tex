...
\todo{Identify limitations as well, i.e. partial algorithm and work duplication. REQUIRED}
...
\todo{Definition style is not used elsewhere, make it consistent}

\todo{Say a program is first order if at runtime it creates no functional values}
...
\paragraph{Introduce no data types.} Reynolds method introduces a new data type that serves as a representation of functions, then embeds an interpreter for this data type into the program. We aim to eliminate the higher-order aspects of a program \textit{without} introducing any new data types. By composing our transformation out of existing transformations, none of which introduces data types, we can easily ensure that our resultant transformation does not introduce data types. \todo{Say why this is important, because it is a bottleneck for analysis}
...
\todo{Conjecture that these rules are confluent including those from the background section}
...
\todo{Explain and justify what template and tem do. Give examples}

\todo{Make the templates much clearer to follow. Justify with helpful examples, i.e. f (\x -> x) 1 and f (\x -> x) 2 give the same template}
...
\todo{Explain the rewrite rules and argue that they faithfully model the termination behaviour of the original rules.}
...
\todo{In its general form, Kruskal's Tree Theorem works for infinite alphabets that are well-quasi ordered Is that applicable here?}
...
\todo{Update with the results from the revised draft paper}
...
\todo{Correct the mislead implication that firstification is responsible for code shrinkage - its probably mainly the simplification stage. Ideally, measure; but that's not a formal requirement.}
