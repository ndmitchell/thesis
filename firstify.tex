...
Our reduction method proceeds in four steps, each removing some higher-order elements, without introducing any data structures. Each step is independently terminating, and are combined using a fixed point operator. Because our method terminates and does not introduce any data structures, it is necessarily incomplete, but Reynold's method can always be used as a backup.
...
Each stage will be elaborated on separately. The operator |(+|+)| is defined as:
...
The operator applies the first argument until it reaches a fixed point, then applies the second argument. If the second argument changes anything, the first operator is tried again until a fixed point is achieved. This allows us to ensure two important properties:

\begin{enumerate}
\item[|f| and |g| are fixed]
\begin{code}
forall f g x `o` let r = (+|+) f g x in f r == r && g r == r
\end{code}
The result of the fixed point will not change if either |f| or |g| is reapplied.

\item[The operator is idempotent]
\begin{code}
forall f g x `o` let r = (+|+) f g x in r == (+|+) f g r
\end{code}

\item[|f| is fixed before |g| is applied] Within the function, before |g| is applied, |f| must have reached a fixed point.
\end{enumerate}

These properties allow us to not worry about overlapping the application sites between |f| and |g|, as |f| is always guaranteed to apply. It also removes the need to determine when the operations have finished, as by the end they will be sorted.
