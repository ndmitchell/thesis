...
Given these assumptions, a lambda or boxed lambda may only occur in the following places: (1) the body of a lambda expression; (2) an argument to a general application; (3) the body of the |main| function. In \S\ref{secF:example_residual} we give examples of these residual forms. Since there is no simple rule to remove lambda expressions from the residual locations, it suggests the algorithm has no obvious deficiencies.
...
Our simplification rules from \S\ref{secB:core_simplify} are applied until a fixed point is found, meaning that the left-hand side of any rule cannot occur in the output. For example, the left-hand side of the (case-con) rule is \ignore|case_ (con s) s|, so this pattern cannot occur within \ignore|s_2|. In view of these rules we can further reduce the bounding set of residual expressions:
...
Having applied all the rules, we now classify what the parent expressions of a lambda may be. A lambda may not be the root of a function body, because |lam| cannot be generated as the root by |s|. If we focus only on those expressions which permit a lambda expression as a direct child, hiding the other expressions with \ignore|...|, we are left with the following upper bound for the set of residual expressions:
...
The residual lambda in |main_1| is a result of the |bottom| function non-terminating, while the lambda in |main_2| is caused by dead code. In both cases the lambda expression will not actually be reached, the no functional value will actually be created at runtime. The final |main_3| example passes a lambda expression to a functional argument to the root function, which could be eliminated by restricting the root function to not accept functional arguments.
