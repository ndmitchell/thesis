...
We identify the following causes of incompleteness:

\begin{enumerate}
\item Passing a lambda expression to a primitive function.
\item Returning a lambda within a constructor box.
\item Passing a lambda expression to a functional value passed in to the root function.
\item Passing a lambda expression to a value that evaluates to |undefined|.
\end{enumerate}

We can prove that these are the only remaining causes of incompleteness.
...
If a top-level function is bound to a lambda expression, then the arity of the function is increased and the lambda is no longer the root expression.
\end{lemma}

\begin{lemma}{The parent of a lambda expression must not be a let expression.}
The rules (bind-lam) and (let-lam) applied as part of |simplify| ensure this condition.
\end{lemma}

\begin{lemma}{The parent of a lambda expression must not be a case expression.}
The (case-lam) rule ensures that an alternative in a case expression is not a lambda expression. The static typing in the original Core language ensures that a lambda cannot be the scrutinee of a case expression.
\end{lemma}

\begin{lemma}{The parent of a lambda expression must not be a lambda expression.}
Using the desugaring rule, and vector lambda expressions, nested lambda expressions can be transformed away.
\end{lemma}

\begin{lemma}{The parent of a lambda must be an application, applied to a function, constructor or variable.}
The (lam-app), (let-app) and (case-app) rules, plus the combination of successive applications, will transform away all other applications.
\end{lemma}

\begin{lemma}{A lambda may not be an argument to a function.}
The |special| rule will transform away all lambda expressions which are arguments to functions.
\end{lemma}

Argument: functions and constructors will be specialised away. If the parent
...
To prove the correctness of the whole argument it is sufficient to show the correctness of each step.

\subsection{Lambda Insertion}
...
However, a closely allied operation that is semantics preserving is:

\begin{code}
\v -> x => \v' -> (\v -> x) v'
\end{code}

The lambda insertion phase can be seen as first transforming the function |f| using the lambda insertion rule, then inlining the outer lambda into the application site. These operations are semantics preserving.

\subsection{Simplification}

Each rule is individually correct with respect to semantics.

\subsection{Inline}

The inlining of functions is safe.
...
To prove termination, each stage must be individually terminating, and there must be no program 
