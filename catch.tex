...
\todo{Less difference between Catch05 and Catch08}

This section describes an automated analysis to check for pattern match errors, which we have called Catch. \S\ref{sec:catch_intro} gives a small example, and \S\ref{sec:walkthrough} gives an overview of the checking process for this example. \S\ref{sec:manipulate} introduces a small core functional language and a mechanism for reasoning about this language, \S\ref{sec:constraint} describes two constraint languages. \S\ref{sec:results} evaluates Catch on programs from the Nofib suite, on a widely-used library and on a larger application program. \S\ref{sec:related} offers comparisons with related work before \S\ref{sec:conclusion} presents concluding remarks.
...
\todo{Define this in terms of the existing Core language}
...
\todo{Move most of these definitions away}
...
pre (Case  on  alts  ) = pre on `propAnd` propAnds (map f alts)
    where f (Alt c vs e) = on -< (ctors c \\ [c]) `propOr` pre e
\end{code}
...
red (Call  fn  xs    ) k =  reduce $ substP xs (prePost fn k)
red (Case  on  alts  ) k =  propAnds [f c e | Alt c vs e <- alts]
    where f c e = reduce (on -< (ctors c \\ [c])) `propOr` red e k
\end{code}
...
loop
    for (fn,k) `elem` p do
        prop'(fn,k) := prop(fn,k) `propAnd` backs (instantiate fn vs `Sat` k)
