...
\todo{Argument for Correctness}

%if 0

\newcommand{\theorem}[2]{#2  \hspace{5mm} \textsf{#1} \\}
\newcommand{\proof}[1]{\paragraph{\textsf{#1}}}
...
There is no |eval| alternative for |Var|, as an expression at evaluation will not have any free variables, as |subst| will remove them. We maintain the correctness of the evaluator is we replace cases which were defined with |Bottom|. We define more restrictive evaluators: We can argue that |evalSafe| will not fail, and requires no additional clauses.
...
\theorem{(pre/eval)}
{|forall x `o` sat (pre x) ==> notBottom (eval x)|}

To do this, we will need to prove the following theorems:

\theorem{(pre/evalPre)}
{|forall x `o` sat (pre x) ==> notBottom (evalPre x)|}
\theorem{(evalPre/eval)}
{|forall x `o` let r = evalPre x in isBottom r |||| (r == eval x)|}
