...
After execution, Catch has determined a precondition for each function, and an entailment, namely a precondition on a function to ensure a particular postcondition. We wish that the preconditions and entailments correctly model the program, and that 

presented in Figures \ref{figC:property_fixp} \ref{figC:precondition} \ref{figC:precond_fixp} \ref{figC:prop} is correct, then that the constraint models are correct.

%if 0
\begin{proof}{|prePost| is correct}

\begin{proof}{|reduce| is correct}
...
\begin{proof}{All functions satisfy their precondition}


\begin{proof}{No value satisfies the precondition to |error|}




\begin{proof}{Given a precondition
\f{Given a precondition no
%endif
