...
\todo{Tough chapter to follow. Try and make clear the separation of constraints and analysis, including which data types belong to each part and what the conditions are. Draw a picture, perhaps?}
...
\todo{Say the problem is undecidable, and encode the halting problem.}
...
\todo{Would be useful to name the different sublanguages of Core that are used here and in the soundness proof.}
...
The |isRec (c,i)| function returns true if the constructor |c| has a recursive |i|th component. For example, let \h{stmt}|hd = (":",0)| and \h{stmt}|tl = (":",1)| then \h{stmt}|isRec hd = False| but \h{stmt}|isRec tl = True|. \todo{Non-recursive selectors must satisfy the property that they occur only some maximum number of times}
...
\todo{Use the same notation as in the appendix}
...
\todo{State the soundness theorem,including any side conditions, and ref the proof in the appendix.}
...
For example, the regular expression |tl^* ^.tl^*| is disallowed as there are two adjacent stars, |hd^*| is disallowed as a non-recursive selector under a star, and |tl| is disallowed as a recursive selector without a star. The restrictions are motivated by three observations:
...
\todo{Tighten up the definition of MP constraints}
