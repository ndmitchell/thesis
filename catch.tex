...
This chapter describes an automated analysis to check for pattern match errors, which we have called Catch. \S\ref{secC:catch_intro} gives a small example, and \S\ref{secC:walkthrough} gives an overview of the checking process for this example. \S\ref{secC:manipulate} introduces a small core functional language and a mechanism for reasoning about this language, \S\ref{secC:constraint} describes two constraint languages. \S\ref{secC:correct} argues for the correctness of the system. \S\ref{secC:results} evaluates Catch on programs from the Nofib suite, on a widely-used library and on a larger application program. \S\ref{secC:catch_related} offers comparisons with related work.
...
\para{Exhaustion:} In the constraint |x `Sat` (r :- [":","[]"])| the condition lists all the possible constructors. Because of static typing, |x| must be one of these constructors. Any such constraint simplifies to True.
...
We do not give a formal proof of correctness, but instead sketch an outline of how such a proof might proceed. We first show that the algorithm is correct given certain conditions on the constraint language, then that each of the three constraint languages satisfy the necessary conditions.
...
The idea is that as we tranform code we leave impossible cases out, which means we can reason about which things to ignore.
...
bottom, notBottom :: Expr -> Bool
bottom Bottom = True
bottom (Val c xs) = any bottom xs
notBottom = not . bottom

sat :: Prop (Sat Expr) -> Bool

satPrecond :: FuncName -> [Expr] -> Bool
\end{code}


\subsubsection{An Evaluator}
...
eval (Call f xs_)  | f == "error" = Bottom
                   | otherwise = eval (call f xs)

call f xs = subst (zip (args f) xs_) (body f)

subst :: [(VarName,Expr)] -> Expr -> Expr
subst r (Var x) = fromMaybe (Var x) (lookup x r)
subst r (Make c xs_) = Make c (map (subst r) xs_)
subst r (Call f xs_) = Call f (map (subst r) xs_)
subst r (Case x as_) = Case (subst r x)
    [Alt n vs (subst r' e) | Alt n vs e <- xs
    ,let r' = filter ((`notElem` vs) . fst) r]
\end{code}

There is no |eval| alternative for |Var|, as an expression at evaluation will not have any free variables, as |subst| will remove them. We maintain the correctness of the evaluator is we replace cases which were defined with |bottom|. In some cases we will be working with predicates which contain no literal values, we use |wrong| to assert that such a predicate is not |true|.

In order to model the proof we assume the existence of a |satisfies| and |satisfiesP| function, much like that given in \S\ref{secC:constraint_semantics}. However, we assume these functions are maximally lazy, to ensure termination in a lazy environment.

\subsubsection{The Statement}

\begin{code}
forall x . pre x ==> notBottom (eval x)
\end{code}

We now proceed by strengthening |eval| so that it returns bottom in more locations. If it is always more like to bottom, and the result is safe, then we are safe.


\subsubsection{Adding precondition checks}

% eliminate evalPrecond?

\begin{code}
evalPrecond (Call f xs_)  | not $ satPrecond f xs_ = Bottom
evalPrecond x = eval x
\end{code}

We also define |eval'| which is like |eval| but does not return |Bottom|.

\subsubsection{Adding |pre| checks}

We can now define |evalPre|:

\begin{code}
evalPre x | not $ sat (pre x) = Bottom
evalPre x = evalSafe x

evalSafe x | sat (pre x) = eval x
\end{code}

We can see that |evalSafe| is fully defined, even though there is some case lacking. As before, |evalPre| is a strengthening of |evalPrecond|.

\subsubsection{Transforming |evalPre|}

We can now transform the first equation in |evalPre|:

\begin{code}
evalSafe x@(Make c xs) | sat (pre x) = evalPrecond x
...
Assuming: |sat (andP xs) = all (sat xs)|.

We can now transform the second equation:

\begin{code}
evalSafe x@(Call f xs) | sat (pre x) = eval x

evalSafe (Call f xs) | not $ satPrecond f xs && all (sat xs) = Bottom
                     | otherwise = eval x
...
evalSafe (Call f xs) | not $ satPrecond f xs = Bottom
                     | not $ all (sat xs) = Bottom
                     | otherwise = eval (call f xs)

evalSafe (Call f xs) | not $ sat $ substP xs $ reduce (pre (body f)) = Bottom
                     | not $ all (sat xs) = Bottom
                     | otherwise = eval (call f xs)

evalSafe (Call f xs) | not $ sat $ subst' s $ reduce' $ pre $ y = Bottom
                     | not $ all sat xs = Bottom
                     | otherwise = eval $ subst s $ y
    where
        s = zip (args f) xs
        y = body f

-- subst/reduce => reduce/subst
evalSafe (Call f xs) | not $ sat $ reduce' $ subst' s $ pre $ y = Bottom
                     ....
...
We know that |precond "error" == false| as the initial value of |precond "error"| is |false|, and the values only get more restrictive, and |false| is the most restrictive value. Therefore |f == "error" ==> wrong (precond "error")|. We can now strengthen the |Call| alternative with:
