...
It is a bad idea to inline cafs. We can remove a caf by adding a dummy argument.

Caf removal is only safe if the function is not strict in the dummy argument, which we insure by adding it in.


\subsection{Recursive let bindings}

A recursive let binding is one where the bound variables are in scope during the computation of their associate expression. In the Haskell language, let bindings can be \textit{recursive}. In other languages, such as ML \cite{ml}, recursive let binding are made explicit with the |letrec| keyword. One common use of |letrec| in Haskell is the |repeat| function:
...
Our Core language does not allow recursive let bindings, for reasons of simplicity. If there is a |letrec| bound to a function, it will be removed by lambda lifting. The only remaining case is a value letrec. Another way of thinking of a value letrec is as a locally scoped CAF. We can remove these letrec's in exactly the same way as CAF's by inserting a dummy argument. Our algorithm is:
...
\item Optionally, remove the inserted markers.
\end{enumerate}

Applying this to our example from before, we first add caf markers:

\begin{code}
repeat x =  letrec xs = \caf ->  x : xs caf
            in xs caf
\end{code}
...
Optionally, we can remove the |caf| annotation:
...
In the case where the |caf| argument is not the only argument to a function, removal of the |caf| annotation to this function is simplifies the code without changing the space behaviour. In the case where it is the only argument, removing the |caf| argument will result in a CAF, which will be computed only once at runtime.
