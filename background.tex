...
\begin{figure}
\begin{code}
data Shell a = Shell a [Shell a]

(<<|) :: Eq a => Shell a -> Shell a -> Bool
a <<| b = dive x y || couple x y

dive x (Shell _ ys) = any (x <<|) ys

couple (Shell x xs) (Shell y ys) =
    x == y && length xs == length ys && and (zip (<<|) xs ys)
\end{code}
...
To ensure that the first field in a |Shell| is drawn from a finite alphabet, we can replace any locally bound variables with the empty string. For example, |shell (EAny v)| would become |Shell (EAny bullet) []|.
