...
The evaluation strategy of our Core language is lazy. We give a semantics based on that of \citet{hammond:semantics}. At each step |Gamma| is the environment containing a mapping from variable names to closures. A closure is a pair of an environment and an expression to be evaluated in that environment. We use the |_F| function to denote the mapping from top-level program names to expressions.

The residual forms in our language are defined by:

\begin{code}
r  =  c \< rs_ \>  {-" \text{  constructor} "-}
   |  \v -> r      {-" \text{  lambda} "-}
   |  undefined    {-" \text{  bottom/undefined} "-}
\end{code}

A residual form is either a constructor, a lambda, or the undefined value. The only way an undefined value may be initially generated is by a call to the error function in the program, i.e. |_F(error) = undefined|. The semantics are given in Figure \ref{fig:semantics}.
