...
\begin{figure}
\def{Str,Zero,One,Two,strList,listStr}
\h{.*}\begin{code}
...
\begin{figure}
\def{children,universe,descend,transform,rewrite,descendM,transformM,rewriteM,para,holes,contexts}
\begin{code}
children :: Uniplate alpha => alpha -> [alpha]
...
\def{childrenOn,universeOn,descendOn,transformOn}
\begin{code}
...
(||+) :: Biplate item alpha => Type ([tau] -> beta) alpha -> [tau] -> Type beta alpha
(||+) (xs,x') y = (Two xs ys, \(Two xs ys) -> x' xs (y' ys))
...
\begin{figure}
\def{PlateTypeable,plateAll,PlateAll,Type,plate,|+,plateSome}
...
To give an instance of |PlateAll beta alpha|, for a concrete type |beta|, we require the context |Typeable alpha| and |Uniplate alpha|. To give an instance where |beta| has type variables of kind |*|, we require the context |Typeable tau| and |PlateAll tau alpha| for all the type variables |tau| in |beta|. We do not permit |beta| to have any higher-kinded type variables. To give an example instance with type variables, the instance for lists is:
...
The code in Figure \ref{figU:platedata} is not optimised for speed. Uses of |splitAt| and |length| require the list of children to be traversed multiple times. Transforming between |Str alpha| and |[alpha]| is also inefficient. We discuss improvements in \S\ref{secU:optimise_playdata}.
...
\begin{onepage}
\def{TypeBox,contains}
\ignore\begin{code}
data TypeBox = forall alpha `o` (Typeable alpha, Data alpha) => TypeBox alpha
...
contains (undefined :: [Int]) =
    [DataBox (undefined :: Int), DataBox (undefined :: [Int])]
\end{code}
