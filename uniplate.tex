...
\todo{State up front, we are not considering functional values}
...
\todo{Define maximal proper substructure and alpha-children}
...
\todo{Make it clear that different strategies may give different results as patterns are allowed to overlap. ie. something like neg (neg x) = 1, applied to neg (neg (neg 1))}
...
\todo{Not just performance sensitive, also semantics sensitive}
...
\todo{Clarify this a lot, give a very precise definition of what is a new constructor, then say that I am using a stronger rule}
...
\todo{Say strategy is fixed to bottom-up}
...
\todo{Give a much stronger specification and description of these two functions, more than just some properties.}

\todo{Connection with the zipper type, add a reference}
...
\todo{Explain how the derived instance declaration is constructed. REQUIRED}
...
\todo{Special rules - say what the rules are}
...
\todo{The reason for not always descending, like uniplate, is that Uniplate processes alpha recursively, but biplate just finds the first uniplate then calls alpha}
...
\todo{Useful to give more of the _On functions, i.e. childrenOn, and also the instances for UniplateExpr on Core/CoreFunc etc.}
...
\todo{Lots more todo here. Explain the role of PlateOne/PlateAll as a mechanism for giving Uniplate instances. Be clear that PlateAll in 3.5.1 is different from 3.5.2. How do these techniques work on parameterised and higher-kinded types. REQUIRED}
...
\todo{Explain the intuition behind databox more, amounts to a mapping from types to all the types found within its values}
