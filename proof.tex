...
sat $ pre $ Call f xs_ ==> sat $ pre $ body f / (args f, xs)
    -- inline pre on LHS
sat $ (precond f / (argsN f / xs_)) `propAnd` propAnds (map pre xs_) ==> sat $ pre $ body f / (args f, xs)
    -- inline |sat|
sat (precond f / (argsN f / xs_)) && all (sat . pre) xs_ ==> sat $ pre $ body f / (args f, xs)
    -- \lemma{precond}
...
\begin{code}
sat (x / (vs, ys)) ==> sat (reduce x /# (vs, ys))
\end{code}
...
% sat (reduce' [[ x ]] ) ==> sat [[ reduce' x ]]


\proof{subst/pre}

% sat [[pre x]] ^ all (sat . pre) xs ==> sat (pre [[x]] )
