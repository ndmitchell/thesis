...
\todo{Give a precise statement of what we are going to prove. Include side conditions such as closed e, belongs to the language, evaluation terminates, constructors have the correct arity. Describe the plan for solving.}

\todo{Issue of meta-circularity. Yours proofs of equivalent of Haskell terms, but that isn't correct because they ignore infinite structures. How should they be considered?}
...
\todo{Expand on ignoring strictness. What issues? Where is it ignored? Why is this safe?}
...
\todo{Say stuff about the correspondence between the Core language here and in previous chapters}

\begin{figure}
\ind{Value,Bottom,Expr,Make,Call,Var,Sel,Case,Alt,eval}
\h{.*}\begin{code}
...
\begin{figure}
\ind{isBottom,valCtor,propIsTrue,propTaut,satE',satE,sat',sat}
\h{.*}\begin{code}
isBottom :: Value -> Bool
...
\todo{State that we proceed by induction on the structure of an expression. Make explicit the induction base, the induction hypothesis and the induction step. Replace circular references to "Soundness Theorem" by induction hypothesis.}
