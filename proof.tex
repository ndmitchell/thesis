...
\newcommand{\lemBPAny}{MP1}
\newcommand{\lemBPMerge}{MP2}
...
\newcommand{\definep}{\subsection*{Proof P1}}
\newcommand{\case}[1]{\paragraph{Case:} #1 \smallskip}
\newcommand{\lemma}[1]{Lemma #1}
\newcommand{\lemmap}{Proof P1}


\hsdef{\begin{comment}
k,f,c,v,i,on,xs_i,x,ys_i,s,a,ws,c',ms_1,ms_2,e
...
This section corresponds to a proof of the code contained in Figure \ref{figC:enumeration}. To perform some of these proofs, we will rely on two auxiliary lemmas about MP-constraints.
...
Testing all values and patterns up to depth 4 (446,105,404 tests) no counter-example is found. We do not believe it is feasible to test this property at a greater depth, but depth 4 gives us reasonable confidence that the property is indeed true.
...
\case{\h{#mp}|k == (ms_1 :* ms_2):ks|, |xs !! i == Bottom|}
...
\case{\h{#mp}|k == (ms_1 :* ms_2):ks|, |xs !! i == Value c' ys|}
...
\case{\h{#mp}|k == (ms_1 :* ms_2):ks|, |xs !! i == Value c' ys|, |isRec (c,i) == False|}
...
\case{\h{#mp}|k == (ms_1 :* ms_2):ks|, |xs !! i == Value c' ys|, |isRec (c,i) == True|}
...
This corresponds to a proof of soundness for the definitions in Figures \ref{figC:property_fixp}, \ref{figC:precondition}, \ref{figC:precond_fixp} and \ref{figC:prop}.
...
We know that any \ignore|Sel x _| value must be contained within an alternative of a \ignore|Case x _| expression. If we assume that the original |Case| expression matched this alternative, we obtain:
...
To write
 ...
 The proof has not been automatically checked. While performing the reasoning steps,


The proof is structured as an evaluator for expressions in our Core language, along with a series of theorems about the relationship between the evaluator, the algorithms and the constraint languages.
