...
\h{exprlist}\begin{code}
satE' (pre (Case x [Alt c vs y]) ==> satE' $ pre $ y / (vs, map uneval xs)
\end{code}

First we work on the RHS only:

\h{exprlist}\begin{code}
satE' $ pre $ y / (vs, map uneval xs)
    -- \lemma{pre}
satE' (pre y /# (vs, map uneval xs)) && all (satE' . pre) (map uneval xs)
    -- |all f . map g == all (f . g)|
satE' (pre y /# (vs, map uneval xs)) && all (satE' . pre . uneval) xs
    -- \lemma{uneval}
satE' (pre y /# (vs, map uneval xs)) && all (not . isBottom) xs
    -- |not (any f xs) == all (not . f) xs
satE' (pre y /# (vs, map uneval xs)) && not (any isBottom xs)
\end{code}

Now we work only on the LHS:

\h{exprlist}\begin{code}
satE' (pre (Case x [Alt c vs y]))
    -- inline |pre|
satE' $ pre x `propAnd` propAnds (map alt [Alt c vs y])
    -- inline |map|
satE' $ pre x `propAnd` propAnds [alt $ Alt c vs y]
    -- inline |propAnds|
satE' $ pre x `propAnd` alt (Alt c vs y)
    -- inline |satE'|
satE' (pre x) && satE' (alt $ Alt c vs y)
    -- \lemma{pre/eval}
not (isBottom $ eval x) && satE' (alt $ Alt c vs y)
    -- |eval x == Val c xs|
not (isBottom $ Val c xs) && satE' (alt $ Alt c vs y)
    -- inline |isBottom|
not (any isBottom xs) && satE' (alt $ Alt c vs y)
\end{code}

We can now combine the two sides, eliminating the |not (any isBottom xs)| from each side:

\h{exprlist}\begin{code}
satE' $ alt $ Alt c vs y ==> satE' (pre y /# (vs, map uneval xs))
    -- inline |alt|
...
We can now return to the whole thing, assuming that the constructor is the first to be chosen. We can reorder the alternatives because they have no order and are processed in a map in the code. We can assume that |eval x| matches \ignore|Val c _| because that branch has been picked.

\h{exprlist}\begin{code}
satE' $ pre $ Case x (Alt c vs y : as) ==> satE' $ pre y
    -- inline |pre|
satE' $ pre x `propAnd` propAnds (map alt (Alt c vs y : as)) ==> satE' $ pre y
    -- inline |satE'|
satE' (pre x) && all (satE' . alt) (Alt c vs y : as) ==> satE' $ pre y
    -- inline |all|
satE' (pre x) && satE' (alt (Alt c vs y)) && all (satE' . alt) as ==> satE' $ pre y
