...
satE' $ (precond f / (args f, xs)) `propAnd` propAnds (map pre xs) ==> f /= "error"
    -- inline |satE'|
satE' (precond f / (args f, xs)) && all (sat . pre) xs ==> f /= "error"
    -- weaken implication
...
satE' (precond f / (args f, xs)) ==> False
    -- implication
not $ satE' $ satE' (precond "error" / (args f, xs))
    -- \lemma{precond/error}
...
\h{exprlist}\begin{code}
sat $ pre $ Call f xs ==> sat $ pre $ body f / (args f, xs)
    -- inline pre on LHS
sat $ (precond f / (args f /# xs)) `propAnd` propAnds (map pre xs) ==> sat $ pre $ body f / (args f, xs)
    -- inline |sat|
sat (precond f / (args f /# xs)) && all (sat . pre) xs ==> sat $ pre $ body f / (args f, xs)
    -- \lemma{precond}
sat ((reduce $ pre $ body f) / (args f /# xs)) && all (sat . pre) xs ==> sat $ pre $ body f / (args f, xs)
    -- \lemma{reduce}
...
\h{exprlist}\begin{code}
sat $ pre $ Case x as ==> sat $ pre x
    -- inline |pre|
sat $ pre x `propAnd` propAnd (map alt as) ==> sat $ pre x
    -- inline |sat|
sat (pre x) && propAnd (map alt as) ==> sat $ pre x
    -- weaken implication
sat $ pre x ==> sat $ pre x
    -- equal
True
