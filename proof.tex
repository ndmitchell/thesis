...
Both of these theorems are given in \S\label{secC:constraint_semantics}, as properties. Note that the definition of |sat| makes use of |(<||)|, so these theorems require the additional two definitions to be consistent with |(<||)|.
...
sat' $ (c <| k) /# ([0..], map (eval . (/ sub)) xs) ==> sat $ Sat (eval $ Make c (map (/ sub) xs)) k
    -- inline |eval| on RHS
sat' $ (c <| k) /# ([0..], map (eval . (/ sub)) xs) ==> sat $ Sat (Value c (map (eval . (/ sub)) xs)) k
    -- fold back |sat|
sat $ Sat (Value c (map (eval . (/ sub)) xs)) k ==> sat $ Sat (Value c (map (eval . (/ sub)) xs)) k
    -- equal
True
\end{code}
...
To perform some of these proofs, we will rely on the following theorem:

\theorem{sat/any}
{\h{#mp}|sat (Sat (Value c xs) ks) == any (\k -> sat $ Sat (Value c xs) [k]) ks|}

\theorem{sat/merge}
{\h{#mp}|sat $ Sat (Value c xs) [merge ms_1 ms_2 :* merge ms_1 ms_2] ==> sat $ Sat (Value c xs) [ms_1 :* ms_2]|}
...
sat' $ propLit $ Sat (Value c xs) [map complete cs :* map complete (ctors (head cs)) | not (null cs)] ==> c `elem` cs
    -- |null cs == False|
sat' $ propLit $ Sat (Value c xs) [map complete cs :* map complete (ctors (head cs)) | not False] ==> c `elem` cs
    -- inline |not|
sat' $ propLit $ Sat (Value c xs) [map complete cs :* map complete (ctors (head cs)) | True] ==> c `elem` cs
    -- simplify list comprehension
sat' $ propLit $ Sat (Value c xs) [map complete cs :* map complete (ctors (head cs))] ==> c `elem` cs
    -- inline |sat'|
sat $ Sat (Value c xs) [map complete cs :* map complete (ctors (head cs))] ==> c `elem` cs
    -- inline |sat|
...
sat' $ propOrs [propAnds $ map propLit $ g vs_1 | Pattern c_1 vs_1 <- map complete cs, c_1 == c] /# ([0..],xs) ==> c `elem` cs
    -- inline |(/)|
sat' $ propOrs [propAnds $ map propLit $ g vs_1 /# ([0..],xs) | Pattern c_1 vs_1 <- map complete cs, c_1 == c] ==> c `elem` cs
    -- inline |sat'|
or [sat' $ propAnds $ map propLit $ g vs_1 /# ([0..],xs) | Pattern c_1 vs_1 <- map complete cs, c_1 == c] ==> c `elem` cs
    -- move the guard
or [c_1 == c && sat' (propAnds $ map propLit $ g vs_1 /# ([0..],xs)) | Pattern c_1 vs_1 <- map complete cs] ==> c `elem` cs
    -- remove the list comprehension
any (\(Pattern c_1 vs_1) -> c_1 == c && sat' (propAnds $ map propLit $ g vs_1 /# ([0..],xs))) (map complete cs) ==> c `elem` cs
    -- |any f (map g xs) = any (f . g) xs|
any ((\(Pattern c_1 vs_1) -> c_1 == c && sat' (propAnds $ map propLit $ g vs_1 /# ([0..],xs))) . complete) cs ==> c `elem` cs
    -- |c `elem` cs == any (== c) cs|
any ((\(Pattern c_1 vs_1) -> c_1 == c && sat' (propAnds $ map propLit $ g vs_1 /# ([0..],xs))) . complete) cs ==> any (== c) cs
    -- lift implication over |any|
(\(Pattern c_1 vs_1) -> c_1 == c && sat' (propAnds $ map propLit $ g vs_1 /# ([0..],xs))) (complete c') ==> c' == c
    -- inline |complete|
c' == c && sat' (propAnds $ map propLit $ g (map (const Any) (nonRecs c')) /# ([0..],xs)) ==> c' == c
    -- weaken implication
c' == c ==> c' == c
    -- equals
True
...
lhs ==> any (\k -> sat $ Sat (Value c' ys) [k]) ((ms_1 :* ms_2):ks)
    -- inline |any|
lhs ==> sat (Sat (Value c' ys) [ms_1 :* ms_2]) || any (\k -> sat $ Sat (Value c' xs) [k]) ks
    -- \lemma{sat/any}
lhs ==> sat (Sat (Value c' ys) [ms_1 :* ms_2]) || sat (Sat (Value c' ys) ks)
    -- reinstate LHS
sat' ((f_lhd $ f_rhd $ ms_1 :* ms_2) /# ([0..],xs)) || sat' (propOrs (map (f_lhd . f_rhd) ks /# ([0..],xs))) ==> sat (Sat (Value c ys) [ms_1 :* ms_2]) || sat (Sat (Value c' ys) ks)
    -- split the implication
(sat' ((f_lhd $ f_rhd $ ms_1 :* ms_2) /# ([0..],xs)) ==> sat (Sat (Value c' ys) [ms_1 :* ms_2])) && (sat' (propOrs (map (f_lhd . f_rhd) ks /# ([0..],xs))) ==> sat (Sat (Value c' ys) ks))
    -- |xs !! i == Value c' ys|
